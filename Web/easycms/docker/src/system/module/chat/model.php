...
class chatModel extends model
{
    ...
    {
        $this->dao->setAutoLang(false)->update(TABLE_USER)->set('clientStatus')->eq($status)->exec();
        return !dao::isError();
    }
    ...
    {
        $chat = $this->dao->setAutoLang(false)->select('*')->from(TABLE_IM_CHAT)->where('type')->eq('system')->fetch();
        if(!$chat)
        {
            $chat = new stdclass();
            ...
            $this->dao->setAutoLang(false)->insert(TABLE_IM_CHAT)->data($chat)->exec();
        }
        return !dao::isError();
    }
    ...
    public function getSignedTime($account = '')
    {
        ...
        $attend = $this->dao->setAutoLang(false)->select('*')->from(TABLE_ATTEND)->where('account')->eq($account)->andWhere('`date`')->eq(date('Y-m-d'))->fetch();
    ...
    {
		$user = $this->dao->setAutoLang(false)->select('id, account, realname, avatar, role, dept, clientStatus, admin, gender, email, mobile, phone, site, qq, deleted')
			->from(TABLE_USER)
			->where('id')->eq($userID)
			->fetch();
    ...
    {
        $dao = $this->dao->setAutoLang(false)->select('id, account, realname, avatar, role, dept, clientStatus, admin, gender, email, mobile, phone, site, qq, deleted')
            ->from(TABLE_USER)
            ->where(1)
    ...
    public function editUser($user = null)
    {
        ...
        $this->dao->setAutoLang(false)->update(TABLE_USER)->data($data)->where('id')->eq($user->id)->exec();
    ...
    public function getMemberListByGID($gid = '')
    {
        ...
        if($chat->type == 'system')
        {
            $memberList = $this->dao->setAutoLang(false)->select('id')->from(TABLE_USER)->where('deleted')->eq('0')->fetchPairs();
        }
        else
        {
            $memberList = $this->dao->setAutoLang(false)->select('user as id')
    ...
    {
        $messages = $this->dao->setAutoLang(false)->select('*')
    ...
    {
        $messages = $this->dao->setAutoLang(false)->select('*')->from(TABLE_IM_MESSAGE)
            ->where('cgid')->eq($cgid)
            ->beginIF($startDate)->andWhere('date')->ge($startDate)->fi()
            ->orderBy('id_desc')
            ->page($pager)
    ...
    {
        $chats = $this->dao->setAutoLang(false)->select('*')->from(TABLE_IM_CHAT)
    ...
    {
        $systemChat = $this->dao->setAutoLang(false)->select('*, 0 as star, 0 as hide, 0 as mute')
            ->from(TABLE_IM_CHAT)
            ->where('type')->eq('system')
            ->fetchAll();

        $chats = $this->dao->setAutoLang(false)->select('t1.*, t2.star, t2.hide, t2.mute, t2.category')
    ...
    {
        $chat = $this->dao->setAutoLang(false)->select('*')->from(TABLE_IM_CHAT)->where('gid')->eq($gid)->fetch();
    ...
    {
        $messages = $this->dao->setAutoLang(false)->select('t2.*')->from(TABLE_IM_MESSAGESTATUS)->alias('t1')
        ...
        $this->dao->setAutoLang(false)->update(TABLE_IM_MESSAGESTATUS)
            ->set('status')->eq('sent')
    ...
    public function update($chat = null, $userID = 0)
    {
        if($chat)
        {
            ...
            $this->dao->setAutoLang(false)->update(TABLE_IM_CHAT)->data($chat)->where('gid')->eq($chat->gid)->batchCheck($this->config->chat->require->edit, 'notempty')->exec();
        }
    ...
    public function setAdmin($gid = '', $admins = array(), $isAdmin = true)
    {
        ...
        $this->dao->setAutoLang(false)->update(TABLE_IM_CHAT)->set('admins')->eq($adminList)->where('gid')->eq($gid)->exec();
    ...
    {
        $this->dao->setAutoLang(false)->update(TABLE_IM_CHATUSER)
            ->set('star')->eq($star)
            ->where('cgid')->eq($gid)
    ...
    public function joinChat($gid = '', $userID = 0, $join = true)
    {
        ...
        {
            /* Join chat. */
            $data = $this->dao->setAutoLang(false)->select('*')->from(TABLE_IM_CHATUSER)->where('cgid')->eq($gid)->andWhere('user')->eq($userID)->fetch();
            if($data)
            {
                ...
                $this->dao->setAutoLang(false)->update(TABLE_IM_CHATUSER)->data($data)->where('cgid')->eq($gid)->andWhere('user')->eq($userID)->exec();
            ...
            $this->dao->setAutoLang(false)->insert(TABLE_IM_CHATUSER)->data($data)->exec();

            $id = $this->dao->lastInsertID();

            $this->dao->setAutoLang(false)->update(TABLE_IM_CHATUSER)->set('`order`')->eq($id)->where('id')->eq($id)->exec();
        }
        else
        {
            /* Quit chat. */
            $this->dao->setAutoLang(false)->update(TABLE_IM_CHATUSER)->set('quit')->eq(helper::now())->where('cgid')->eq($gid)->andWhere('user')->eq($userID)->exec();
        }
        return !dao::isError();
    }
    ...
    public function createMessage($messageList = array(), $userID = 0)
    {
        ...
        {
            $msg = $this->dao->setAutoLang(false)->select('*')->from(TABLE_IM_MESSAGE)->where('gid')->eq($message->gid)->fetch();
            if($msg)
            {
                if($msg->contentType == 'image' || $msg->contentType == 'file')
                {
                    $this->dao->setAutoLang(false)->update(TABLE_IM_MESSAGE)->set('content')->eq($message->content)->where('id')->eq($msg->id)->exec();
                }
                $idList[] = $msg->id;
            }
            elseif(!$msg)
            {
                ...
                $this->dao->setAutoLang(false)->insert(TABLE_IM_MESSAGE)->data($message)->exec();
                ...
                $data->status = 'sent';
                $this->dao->setAutoLang(false)->insert(TABLE_IM_MESSAGESTATUS)->data($data)->exec();
            }
            $chatList[$message->cgid] = $message->cgid;
        }
        if(empty($idList)) return array();

        $this->dao->setAutoLang(false)->update(TABLE_IM_CHAT)->set('lastActiveTime')->eq(helper::now())->where('gid')->in($chatList)->exec();
    ...
    public function saveOfflineMessages($messages = array(), $users = array())
    {
        foreach($users as $user)
        {
            foreach($messages as $message)
            {
                ...
                $data->gid    = $message->gid;
                $data->status = 'waiting';
                $this->dao->setAutoLang(false)->replace(TABLE_IM_MESSAGESTATUS)->data($data)->exec();
            }
        }
        return !dao::isError();
    }
    ...
    public function getOfflineNotify($userID)
    {
        ...
        $this->dao->setAutoLang(false)->update(TABLE_IM_MESSAGESTATUS)->set('status')->eq('sent')->where('gid')->in($gids)->andWhere('user')->eq($userID)->exec();
        return $notify;
    }
    ...
    public function offlineUser($offline = array())
    {
        ...
        $this->dao->setAutoLang(false)->update(TABLE_USER)->set('clientStatus')->eq('offline')->where('id')->in($offline)->exec();
        return !dao::isError();
    }
    ...
    public function sendFailMessage($sendfail = array())
    {
        foreach($sendfail as $userID => $gid)
        {
            ...
            $idList   = $this->dao->setAutoLang(false)->select('id')->from(TABLE_IM_MESSAGE)->where('gid')->in($gid)->fetchPairs();
    ...
    public function createNotify($target = '', $title = '', $subtitle = '', $content = '', $contentType = 'text', $url = '', $actions = array(), $sender = 0)
	{
		...
		$this->dao->setAutoLang(false)->insert(TABLE_IM_MESSAGE)->data($notify)->exec();
    ...
    {
        $data = $this->dao->setAutoLang(false)->select('id')->from(TABLE_ACTION)
    ...
    public function getExtensionList($userID)
    {
        ...
        $entryList = $this->dao->setAutoLang(false)->select('*')->from(TABLE_ENTRY)->orderBy('`order`, id')->fetchAll();
        $files     = $this->dao->setAutoLang(false)->select('id, pathname, objectID')->from(TABLE_FILE)->where('objectType')->eq('entry')->fetchAll('objectID');
